...
def _get_structured_morph_meta(metacsv):
    ...
    for line in csv_array[1:]:  # Exclude header
        data[line[0]] = {
            "cropped": line[0],
            "apparent_age": line[1],  # Adjust indices based on Morph-II CSV format
            "real_age": line[2],
            "roi": get_roi_morph(line[3:7]),
        }
    return data

def structured_morph_data_wrapper(partition):

    """Wraps the metadata loading function for different partitions."""
    metacsv = 'morph_II/gt_avg_<part>.csv'
    ...
    return _get_structured_morph_meta(metacsv)

def _load_ages(metacsv, partition):

    """Loads age data for a specific partition."""
    global morph_ages
    if morph_ages is None or partition not in morph_ages or morph_ages[partition] is None:
        morph_ages[partition] = _get_structured_morph_meta(metacsv)
...
def get_age_from_morph(path, metacsv, partition):

    """Retrieves age and ROI data for a given image path."""
    _load_ages(metacsv, partition)
    try:
        cropped = morph_ages[partition][path]['cropped']
        age = morph_ages[partition][path]['apparent_age']
        roi = morph_ages[partition][path]['roi']
...
def _load_dataset(meta, csvmeta, imagesdir, partition, cropped=True):
    ...
    for item in tqdm(meta[1:]):  # Exclude header
        image_path = item[0]
        cropped_image_path, apparent_age, roi = get_age_from_morph(image_path, csvmeta, partition)
...
def _load_morph(csvmeta, imagesdir, partition, debug_max_num_samples=None):

    """Loads the Morph-II dataset for a specific partition."""
    metapartition = get_metapartition_label(get_partition_label(partition))
    print("Directory partition:", metapartition)
    morph_partition_dir = imagesdir.replace('<part>', metapartition)
    morph_partition_csv = csvmeta.replace('<part>', metapartition)
    morph_partition_meta = _readcsv(morph_partition_csv, debug_max_num_samples)
    print("CSV {} read complete: {} samples".format(morph_partition_csv, len(morph_partition_meta)))
    return _load_dataset(morph_partition_meta, morph_partition_csv, morph_partition_dir, partition)

class MorphAge:

    """Class for handling Morph-II dataset loading and preprocessing."""
    def __init__(self,
                 partition='train',
                 imagesdir='morph_II/<part>',
                 csvmeta='morph_II/gt_avg_<part>.csv',
        ...
        cache_file_name = 'morph_age_{method}_{partition}{num_samples}.cache'.format(method=method, partition=partition, num_samples=num_samples)
        ...
        try:
            ...
            if partition == "all":
                ...
                for partition in ["train", "val", "test"]:
                    ...
                    self.data.extend(_load_morph(csvmeta, imagesdir, partition, debug_max_num_samples))
            else:
                print("Loading %s data from scratch" % partition)
                csvmeta = os.path.join(EXT_ROOT, csvmeta